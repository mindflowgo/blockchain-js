...
export function urlCall({body, ...options}) {
    return new Promise((resolve,reject) => {
        ...
        // console.log( `<< [${options.nodeAuth} >> ${options.hostname.slice(7)}] ${options.path} ${method}` )
        const req = http.request({
            method,
            headers: {
                // 'Authorization': `Bearer ${process.env.SENTRY_AUTH_TOKEN}`,
                'NodeToken': `${options.nodeAuth || 'API'}`, // TODO nodeAuth will some authenticated token
...
export function handleGET(handler, nodeState) {
    return (res, req) => {
        ...
        req.nodeAuth = req.getHeader('nodeauth') // TODO proper auth'd node-id
        ...
        if( nodeState === 'ONLINE' || nodeState === 'PREPARING' ){ // || ['/blocks/hashes','/blocks','/node/announce'].includes(req.url) 
            ...
            debug( 'red', `>> [${req.nodeAuth}]${req.url} Ignoring (in state: ${nodeState})}` )
...
export function handlePOST(handler, nodeState) {
    return async (res, req) => {
        ...
        const head = { // can't pass req into async body so new object
            nodeAuth: req.getHeader('nodeauth'), // TODO proper auth'd node-id
        ...
        if( nodeState === 'ONLINE' || nodeState === 'PREPARING' || ['/transactions'].includes(req.url) ){
            ...
            debug( 'red', `>> [${head.nodeAuth}]${head.url} Ignoring (in state: ${nodeState})}` )
            res.end( JSON.stringify({ error: `Currently not online: ${nodeState}` }) )
        }
    }
}