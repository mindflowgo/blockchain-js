...
export function handleGET(handler, nodeState) {
    return (res, req) => {
        ...
        if( nodeState === 'ONLINE' ){ // || ['/blocks/hashes','/blocks','/node/announce'].includes(req.url) 
            handler(res, req)
        } else {
            debug( 'red', `${req.url} DENIED (state: ${nodeState})` )
...
// const createPostHandler = (handler) => {
    return async (res, req) => {
        ...
        // if node state is not online, adjust behaviour
        if( nodeState === 'ONLINE' || ['/transactions'].includes(req.url) ){
            try {
            ...
            debug( 'red', `${head.url} DENIED (state: ${nodeState})` )
            res.end( JSON.stringify({ error: `Currently not online: ${nodeState}` }) )
        }
    }
}