...
class Queue {
    ...
    add( transaction ){
        ...
        if( this.Hashes.exists(transaction.hash) ){
            debug('dim',`    -> +re-saving to mempool:[Queue.add]   ${this.Wallet.getNameOnly(transaction.src) || transaction.src}/${transaction.seq} -> ${this.Wallet.getNameOnly(transaction.dest) || transaction.dest} [${transaction.token}]${transaction.amount}; hash:${transaction.hash} already pending, skipping re-adding` )
            return { error: false }
        }
        ...
        debug('dim',`    -> [Mempool::Queue.add]   ${this.Wallet.getNameOnly(transaction.src) || transaction.src}/${transaction.seq} -> ${this.Wallet.getNameOnly(transaction.dest) || transaction.dest} [${transaction.token}]${transaction.amount}` )
        return { error: false }
    }
    ...
    getMinerSorted({ miner, maxTransactions }){
        ...
        let mineTransactions = this.filter('miner',miner)
            ...
            // .slice()
            console.log( `[getMinerSorted] mineTransactions: `, mineTransactions )
...
console.log( `[Mempool::delete] hashes:`, hashes )
        ...
        // DEBUG output
        debug('cyan',`    ~ [Mempool.Queue.delete()] hashes(${hashes?.length || '0'}) timestamp(${timestamp||'-'}) this.queue(${this.queue.length})` )
