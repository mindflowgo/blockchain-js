...
class Hashes {
    ...
    updateBlockIdx( hash, idx ){
        if( !this.exists(hash) ) {
            ...
            throw errorWithData( `Hash already on-block.`,  { index: this.findBlockIdx(hash) } )

        } else if( this.findBlockIdx(hash) > -1 && this.findBlockIdx(hash) !== idx ) {
            throw errorWithData( `Hash already used by ANOTHER transaction, this attempted hash update ignored.`, { index: idx } )
...
class Queue {
    ...
    purgeStale( purgeSince ){
        ...
        const { hashes, transactions }= this.delete( undefined, purgeSince )
    ...
    add( transaction ){
        ...
        if( srcCnt >= MAX_PENDING_PER_USER )
            throw errorWithData( `Sorry too many transactions (${srcCnt}) by user (${transaction.src}). Please resubmit later once blocks mined.` )
    ...
    delete( hashes, timestamp = '' ){
        ...
        if( hashes && hashes.length > 0 ){
            ...
            throw errorWithData( "No hash/timestamp given" )
        }

        deleteHashes = deleteTransactions.map( t => t.hash )
        if( deleteTransactions.length < 1 || deleteHashes.length<1 ) throw errorWithData( "No transactions to purge" )
        ...
        for( const hash of deleteHashes )
            if( this.Hashes.findBlockIdx(hash) === -1 ){
                debug( 1,`<red>ERROR:</>~ YES this.hashes[${hash}].index = -1, so removing the hash! ` + (deleteHashes.includes(hash) ? `(and already deleted matching transaction)` : `(but couldn't find matching queue transaction)`) )
                this.Hashes.delete(hash)
            }

        return { hashes: deleteHashes, transactions: deleteTransactions }
    }
}
