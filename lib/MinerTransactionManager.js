...
export default class TransactionManager {
    ...
    new({src, dest, amount, fee = 0, seq = 0, txSig = '', hash = '', ...data}, options = {} ) {
        ...
        const { error: hashError, isNew, index }= this.findOrCreateHash( hash, blockIdx )
        ...
        if( isNew ){
            ...
            // now if we got t
            // he error with pending posted on multiple servers, lets' have that warning appear here
            if( warning ){
    ...
    getPending({ src, ofMiner, maxTransactions, metaInclude=true }){
        ...
        let mineTransactions = this.pending
            .filter(t => (ofMiner && t.meta.miner === ofMiner) || (src && t.src === src) )
