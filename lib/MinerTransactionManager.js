...
export default class TransactionManager {
    ...
    newBatch( _transactions, options = {} ){
        ...
        for( const _transaction of _transactions ){
            ...
            if( transaction.error ){
                if( transaction?.index<0 ){
                    ...
                    debug( 'red', ` .. transaction REAL error (${transaction.error}), deleting from hashes[]`)
        ...
        return { error: false, result, transactionCnt, hashes, newHashes }
    }
    ...
    transactionReverse({src, dest, amount, fee, seq, hash, ...data}, options = {}) {
        const { blockIdx = -1, txUpdate = false } = options

        if( !hash || hash.length<40 ) return { error: `Invalid hash!`}
        
        this.ledger.transaction({src, dest, amount, fee, seq, hash, ...data}, { blockIdx, txUpdate, reverse: true })
...
return false
        _transactions.forEach( t => {
            ...
            const result = this.transactionManager.reverse( t )
            if( result.error )
    ...
    deletePending({ hashes, timestamp, reverse = false }){
        ...
        if( 0 && reverse && deleteHashes.length > 0 ){
            for( let t of deleteHashes ){
                debug('cyan',`   ~ Deleting pending transactions: ${t.src.split(':')[0]}/${t.seq} > ${t.dest.split(':')[0]} $${t.amount} Staked(${t?.txStake})`)
                this.reverse(t, {clearPending: true})
            }
        }

        // now remove those delete ones
        this.pending = this.pending.filter( t => !deleteHashes.includes(t.hash) )
