...
export default class TransactionManager {
    constructor({ nodeName, ledger }) {
        ...
        this.transactionFee = Math.round(TRANSACTION_FEE_PERCENT*1000/100000)
        this.transactionFeeCap = TRANSACTION_FEE_CAP
    ...
    getFee({ amount, fee=0 }){
        ...
        if( this.transactionFeeCap > 0 )
            fee = Math.min(this.transactionFeeCap, fee)
        return fee
    }
    ...
    deletePending({ hashes, timestamp = '' }){
        ...
        if( hashes && hashes.length > 0 ){
            ...
            deleteTransactions = this.pending.filter( t => t.timestamp < timestamp )
        ...
        deleteTransactions.forEach( t=>{
            debug('dim',`      ~ ${t.src.split(':')[0]}/${t.seq} >> ${t.dest.split(':')[0]} $${t.amount} [${t.type}] ${timestamp ? `staled(${timestamp-t.timestamp})` : ''} -> deleted.`)
        })
