...
export default class Block {
    ...
    static buildGenesis(supply, TransactionHandler) {
        const genesisTransactions = TransactionHandler.createToken(supply)
    ...
    constructor({ index = 0, prevHash = '0', version = '', timestamp = time(), minerName = Miner.nodeName, merkleRoot = '0', nonce = 0, 
                  powTime = 0, transactions = [] }, options = {}){ 

        const { readOnly = false, forceOverwrite = false } = options
        ...
        if( forceOverwrite ){
            ...
            if( readBlockData ){
                ...
                return { error: `Read-only request (#${this.index}) but no block exists, failing!` }
            }
        }
    }
    ...
    mine(difficulty = 1, iterations = undefined) {
        ...
        do {
            ...
            // if( hash.startsWith(hashPrefix) && !hash.startsWith(hashPrefix+'0') ) break
