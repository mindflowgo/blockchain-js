...
export default class Block {
    ...
    isHashValid(difficulty, hash = this.hash) {
        ...
        return hash.startsWith(hashPrefix) && !hash.startsWith(`${hashPrefix}0`) && this.hash === hash
    }
    ...
    mine(difficulty = 1, iterations = undefined) {
        ...
        do {
            ...
            console.log( `block::mine (diff: ${difficulty}) this.nonce+(${this.nonce}) hash(${hash})`)
        ...
        this.hash = hash
        if( this.isHashValid(difficulty) ){
            return true
        }  else {
            return false
        }
