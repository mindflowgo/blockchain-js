...
import { time, wait, debug, errorWithData } from './helper.js'
    ...
    async mineBlock( blockData ) {
        return new Promise((resolve,reject) => {
            ...
            this.node.on('message', ({ action, ...result }) => {
                switch( action ){
                    case 'UPDATE':
                        debug( 3,`~ [Miner] Worker update (nonce=${result.nonce}, elapsed=${result.elapsed}s)`)
                        this.elapsed = result.elapsed
                        break
                    case 'DONE_TIMEOUT':
                    case 'DONE_ABORTED':
                    case 'DONE_UNSOLVED':
                        debug( 3,`~ [Miner] Worker notified ${action}; elapsed=${result.elapsed}s. Ready again.`)
                        this.status = 'IDLE'
                        resolve({action})
                        break
                    case 'DONE_SOLVED':
            ...
            this.node.on('error', (error) => {
                debug( 1, '<red>~ [Miner] Worker ERROR:', error)
                reject(error)
            })

            this.node.on('exit', (code) => {
                debug(1, `<red>~ [Miner] Worker DIED with exit code ${code}`); // code 0 = normal
