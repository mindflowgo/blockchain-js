...
async function mineBlock(block, difficulty, nonceStart, nonceIterations = 1000000) {
    const start = time()
    mining = true

    console.log( `\n .. [Worker] starting mining #${block.index} (difficulty: ${difficulty}) nonceIterations(${nonceIterations})` )
    ...
    do {
        ...
        if( block.nonce > (Number.MAX_SAFE_INTEGER - 2*nonceIterations) ) break
        nonceFound = block.mine( difficulty, nonceIterations )
...
parentPort.on('message', async ({ action, ...data }) => {
    switch( action ){
        ...
        case 'MINE':
            ...
            const nonceIterations = data.nonceIterations
            ...
            const mineResult = await mineBlock(block, difficulty, nonceStart, nonceIterations)
            const blockData = mineResult.block.getData()
            const elapsed = mineResult.error
            if( !mineResult.error )
                parentPort.postMessage({ action: 'SOLVED', elapsed, blockData })
            else if( mining ) // error likey because failed in nonse-scan range
                parentPort.postMessage({ action: 'UNSOLVED', elapsed, nonceStart, nonceIterations })
            else
                parentPort.postMessage({ action: 'ABORT', elapsed, nonceStart, nonnonceIterationsceEnd })
