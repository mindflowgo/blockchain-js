...
export default class Wallet {
    ...
    findUsers(match) {
        return Object.keys(this.addresses).filter( publicKey => 
                this.addresses[publicKey].name.indexOf(match)> -1 && 
                this.addresses[publicKey].name !== this.TransactionHandler.MINT_TOKEN )
    }
    ...
    getUser(name, autoCreate = true) {
        ...
        if( this.addresses[publicKey] === undefined && autoCreate){
            ...
            this.resetAllTokens(publicKey)
            this.update(name)
            debug('dim', `[getUser] Ccreated user (name: ${name}, publicKey(${publicKey}))` )
        }
    ...
    generate(name) {
        ...
        if( wallet.error ){
            ...
            this.addresses[publicKey] = { name, created: time(), publicKey, privateKey }
            this.resetAllTokens(publicKey)
    ...
    update(name, walletData={}) {
        ...
        if( !this.addresses[publicKey] ) this.addresses[publicKey] = {}
        ...
        if( Object.keys(walletData).length > 0 ){
            ...
            // const tokenBalance = {}
            // const tokens = Object.keys(walletData).filter(k => k.endsWith('$'))
            // for( const token of tokens ){
            //     tokenBalance[token].tx = walletData[token]?.tx ? { ...wallet[token]?.tx, ...walletData[token].tx } : wallet[token].tx
            //     tokenBalance[token].onChain = walletData[token]?.onChain ? { ...wallet[token]?.onChain, ...walletData[token]?.onChain } : wallet[token].onChain
            // }
            // const tx = walletData?.tx ? { ...wallet.tx, ...walletData?.tx } : wallet.tx
            // const onChain = walletData?.onChain ? { ...wallet.onChain, ...walletData?.onChain } : wallet.onChain
            // wallet = { ...wallet, ...walletData, ...tx, ...onChain, ...tokenBalance, publicKey }
            wallet = { ...wallet, ...walletData, publicKey }
