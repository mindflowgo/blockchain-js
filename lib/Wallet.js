...
export default class Wallet {
    constructor( walletFile, TransactionHandler ) {
        this.addresses = {}
        this.snapshots = {}
    ...
    buildNameWithPublicKey(name, publicKey = '') {
        ...
        if( publicKey && !name.endsWith('$') ){
            // create entry for name
            if( !this.addresses[publicKey] ) 
                this.createUser(name, publicKey)
            
            // if necessary, join name + publicKey
            if( name !== publicKey && !name.includes(':') )
                name += ':' + publicKey
        }
        return name
    }
    ...
    getUserPublicKey(name) {
        ...
        if( !name ) {
            throw errorWithData('getUserPublicKey: Empty name')
        } else if( typeof(name)==='object' ) {
            if( !name.publicKey ) throw errorWithData( 'Wallet without publicKey' )
        ...
        if( publicKey.length < 1 ){
            ...
            throw errorWithData( `Public Key for ${name} NOT found in local wallets` )
        } else if( publicKey.length !== 45 ) 
            throw errorWithData( `Public Key (${publicKey}) length (${publicKey.length}) for ${name} wrong.` )

        if( Crypto.keyChecksum(publicKey) !== publicKey.slice(-1) ){ // last digit is the checksum digit
            throw errorWithData( `Public Key for ${name} INVALID checksum, typo?` )
        }
    ...
    getDefaultUser() {
        ...
        const defaultUser = addresses.filter( w => w.default )
        if( defaultUser && defaultUser[0] ) return defaultUser[0]

        // no default one, so use first entry
        return addresses[0]
    }

    getUserOrCreate(name) {
        return this.getUser(name, true)
    }

    getUser(name, autoCreate = false) {
        ...
        if( wallet === undefined && autoCreate){
            ...
            wallet = this.createUser(name,publicKey)
        }

        if( !wallet ) throw errorWithData( `User ${name} didn't exist; and autoCreate disabled. Aborting.`, { name } )

        return wallet
    }

    createUser(name, publicKey) {
        // place name/seq as we want object key order consistent for ease of reading
        this.addresses[publicKey] = { created: time(), publicKey, name: null, seq: null }
        ...
        this.update(name)
        debug('4', `  ~ [createUser] Created new user: ${name}, publicKey: ${publicKey}` )
        return wallet
    }
    ...
    generate(name) {
        if( !name || name.includes('$') || name.includes(':') || name.startsWith('*') || name.length === 45 ) 
            throw errorWithData( `You need to give a name for the wallet address (cannot *start* with (*); cannot have a dollar-sign($) or colon (:) in it)` )

        // if user exists, check if privateKey, if not, we add it, if no user, we create that user too!
        let wallet
        try {
            wallet = this.getUser(name)
            throw errorWithData( `Name '${name}' already taken, select another to generate.` )
        
        } catch( e ) {
            // wallet doesn't exist, generate it
            debug( 4,`<dim>- No existing publicKey or privateKey for (${name}); creating public/privateKeys</>`)
            let { publicKey, privateKey }= Crypto.genKeyPair()
            publicKey += Crypto.keyChecksum(publicKey)  // append a checksum character
            wallet = this.getUserOrCreate(name+':'+publicKey)   // now auto-generate wallet with publicKey
    ...
    update(name, walletData={}) {
        ...
        let wallet = this.addresses[publicKey]
        if( !wallet ) throw errorWithData( `For [update], name(${name}) non-existent wallet` )
    ...
    userRestores( names ){
        ...
        for( const name of names ){
            ...
            delete this.snapshots[publicKey]
            restoreCnt++
        }
        return { restoreCnt }
    }
        ...
        if( compact ){
            ...
            for( const w of addresses.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)) ){
                ...
                for( const token of this.listTokens(w) ){
                    // console.log( ` token(${token}) of ${w.name}`)
                    if( name.length!==0 && w[token]?.tx.balance>0 ){
                        ...
                        debug('dim',`   - ${name}${seqInfo}${' '.repeat(20-(token.length+seqInfo.length+name.length))} ${token}${w[token].tx.balance || '0'} `
                                +`${' '.repeat(20-(w[token].tx.balance||'0').toString().length)} ${token}${onChainBal} `
    ...
    sign(name, data) {
        if( name.startsWith('*root:') ) {
            if( !BLOCKCHAIN_PRIVATEKEY || BLOCKCHAIN_PRIVATEKEY.length<50 )
                throw errorWithData( `Unable to sign as *root, BLOCKCHAIN_PRIVATEKEY missing/invalid in .env` )
            return '*root:' + Crypto.sign(BLOCKCHAIN_PRIVATEKEY, data)
        }

        const wallet = this.getUser(name)
        if( !wallet.privateKey ) 
            throw errorWithData( `Unable to sign transaction: Missing privateKey for (${name}); Declining.` )

        return this.getNameOnly(name) + ':' + Crypto.sign(wallet.privateKey, data)
    }
    ...
    decode(name, signedData) {
        if( name.startsWith('*root:') ) {
            if( !BLOCKCHAIN_PUBLICKEY || name !== `*root:${BLOCKCHAIN_PUBLICKEY}` )
                throw errorWithData( `Unable to decode as *root, publicKey missing/invalid in .env` )
            return Crypto.decode(BLOCKCHAIN_PUBLICKEY, signedData)
        }

        const wallet = this.getUser(name)
        if( !wallet.publicKey ) 
            throw errorWithData( `Unable to decode transaction: Missing publicKey for (${name}); Declining.` )
    ...
    // when rebuilding ledger, we reset all tokens for users to 0
    resetAllTokens( addresses = Object.keys(this.addresses) ){
        ...
        for( const publicKey of addresses ){
            ...
            for( const token of this.listTokens(wallet) )
                wallet = this.initToken(wallet,token)
        }
    }
}
