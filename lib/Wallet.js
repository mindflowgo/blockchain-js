...
export default class Wallet {
    ...
    getUser(name, autoCreate = false) {
        ...
        return wallet || { error: `User ${name} didn't exist; and autoCreate disabled. Aborting.`}
    }
    ...
    createUser(name, publicKey) {
        ...
        this.update(name)
        debug('dim', `  ~ [createUser] Created new user: ${name}, publicKey: ${publicKey}` )
        return wallet
    }
    ...
    generate(name) {
        if( !name || name.includes('$') || name.includes(':') || name.startsWith('*') || name.length === 45 ) 
            return { error: `You need to give a name for the wallet address (cannot *start* with (*); cannot have a dollar-sign($) or colon (:) in it)` }
    ...
    sign(name, data) {
        if( name.startsWith('*root:') ) {
            if( !BLOCKCHAIN_PRIVATEKEY || BLOCKCHAIN_PRIVATEKEY.length<50 )
                return { error: `Unable to sign as *root, BLOCKCHAIN_PRIVATEKEY missing/invalid in .env`}
    ...
    decode(name, signedData) {
        if( name.startsWith('*root:') ) {
            if( !BLOCKCHAIN_PUBLICKEY || name !== `*root:${BLOCKCHAIN_PUBLICKEY}` )
                return { error: `Unable to decode as *root, publicKey missing/invalid in .env`}
