...
export default class Ledger {
    ...
    createWallet(name) {
        ...
        if( !userWallet.privateKey && !userWallet.publicKey){
            debug('dim',` - No existing privateKey for (${name}); creating public/privateKeys`)
        ...
        for( const walletAddress of walletAddresses ){
            ...
            if( blockIdx > -1 ) {
                ...
                if( !reverse ){
                    ...
                    // BUGBUG REVIEW assuming all transactions in that block being reversed, so remove block
                    if( onChain.historyIdx.includes(blockIdx) ) onChain.historyIdx.shift() 
                    if( idx===0 ) onChain.seq = Math.max(onChain.seq - 1, 0)
                    if( idx===0 ) tx.seq = Math.max(tx.seq - 1, 0, onChain.seq)
                }
    ...
    walletBalances(names = [], compact = false ) {
        ...
        if( compact ){
            ...
            wallets.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
                ...
                if( !(names.length===0 && name === '_') && i.tx.balance>0 ){
                    ...
                    const onChainDepth = i.onChain.historyIdx.length > 0 ? this.maxBlockIdx - i.onChain.historyIdx[0] : 0
                    debug('dim',`   - ${name}${seqInfo}${' '.repeat(20-(seqInfo.length+name.length))} $ ${i.tx.balance || '0'} `
                               ...
                               +`${' '.repeat(18-(onChainDepth).toString().length)} ${onChainDepth}`
                               +`${' '.repeat(10-(onChainBal).toString().length)} ${i.onChain.historyIdx || ''}`)
        ...
        // add in depth
        return wallets.map( ({ privateKey, ...data }) =>{ return { ...data, depth: data.onChain.historyIdx.length > 0 ? this.maxBlockIdx - data.onChain.historyIdx[0] : 0 } } )
    }
