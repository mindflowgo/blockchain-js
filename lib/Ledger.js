...
export default class Ledger {
    ...
    buildTransactionName(name, publicKey = '') {
        if( !publicKey ) publicKey = this.getPublicKey(name)
    ...
    getWallet(name) {
        ...
        if( this.wallets[publicKey] === undefined )
            this.wallets[publicKey] = { name, publicKey, seq: 0, balance: 0 }
    ...
    transactionSign({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( hash !== '' && hash !== newHash )
            return { error: `Transaction had a hash (${hash}) that did not match our calculated hash(${newHash}). Rejecting.` }
        ...
        if( src.startsWith('_') ) { //a)
            ...
            if( srcWallet.error || !srcWallet.privateKey ) 
                return { error: `Unable to sign transaction: ${!srcWallet.privateKey ? 'Missing privateKey': srcWallet.error}. Rejecting.` }
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( data.reverseAuth ){
            ...
            // no sequence given? then take users sequence + 1
            // const srcSeq = this.getTransactionSeq( srcWallet.publicKey )
            // console.log( `srcWallet.publicKey(${srcWallet.publicKey}) srcSeq(${srcSeq})` )
            // if( srcSeq.error ){
            //     // create wallet if none
            //     this.updateWallet(src)
            //     seq = 1
            //     console.log( ` was no user (${publicKey}), created updateWallet(${src})`, this)
            // }
            // no seq, use the users last + 1
            if( seq < 1 ) seq = srcWallet.seq + 1
            ...
            if( src !== '_' && Number(srcWallet.balance) < (amount+fee) ){
                ...
                return { error: `Known ${src} balance(${srcWallet.balance}) less than transaction amount(${amount}+fee=${amount+fee}). Rejecting.`, balance: Number(srcWallet.balance) }
            }
        ...
        console.log( `  ~ transaction ${data.reverseAuth?'*REVERSED*':''} (${src.split(':')[0]}/${seq}${srcWallet.privateKey?'[Signed]':''})`
    ...
    walletBalances(names = []) {
        ...
        wallets.forEach( i=>{
            ...
            if( !(names.length===0 && i.name === '_') )
                console.log( `   - ${name}: ${' '.repeat(20-name.length)} $ ${i.balance || '0'} ${' '.repeat(20-i.balance.toString().length)}`)
