...
export default class Ledger {
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( data.reverseAuth ){
            ...
            if( seq > 0 && seq !== srcWallet.seq + 1 ){ // TODO should queue it up and look later (ex. if out of order)
                console.log( `Wallet for (${srcWallet.name}) expects next seq(${srcWallet.seq+1}), but transaction seq(${seq}). Missing transactions? Rejecting.` )
                return { error: `Wallet for (${srcWallet.name}) expects next seq(${srcWallet.seq+1}), but transaction seq(${seq}). Missing transactions? Rejecting.`, balance: Number(srcWallet.balance) }
    ...
    walletBalances(names = []) {
        ...
        wallets.sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
            ...
            if( !(names.length===0 && i.name === '_') )
                console.log( `   - ${name}/${i.seq}:${i.seq>9 ? (i.seq>99 ? '' : ' ') : '  '} ${' '.repeat(20-name.length)} $ ${i.balance || '0'} ${' '.repeat(20-i.balance.toString().length)}`)
