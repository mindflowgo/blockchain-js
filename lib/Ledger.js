...
export default class Ledger {
    ...
    reset(){
        ...
        addresses.forEach( publicKey => {
            ...
            this.wallets[publicKey].blockSeq = { seq: 0, blockIdx: [] }
            this.wallets[publicKey].tx = { seq: 0, amount: 0, hash: '' }
        })
    }
    ...
    getPublicKey(name) {
        ...
        if( publicKey.length < 1 ){
            console.log( `Public Key for ${name} NOT found in local wallets` )
    ...
    getWallet(name) {
        ...
        if( this.wallets[publicKey] === undefined )
            this.wallets[publicKey] = { name, created: time(), publicKey, blockSeq: {seq: 0, blockIdx: []}, balance: 0, tx: {seq: 0} }
    ...
    updateWallet(name, walletData={}) {
        ...
        const tx =  { seq: 0, ...this.wallets[publicKey]?.tx, ...walletData?.tx }
        const blockSeq = { seq: 0, blockIdx: [], ...this.wallets[publicKey]?.blockSeq, ...walletData?.blockSeq }
        const wallet = { ...this.wallets[publicKey] || { balance: 0 }, ...walletData, blockSeq, tx, publicKey }
    ...
    transactionSign({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( hash !== '' && hash !== newHash ){
            if( this.outputDebug ) console.log( ` [transactionSign] Transaction passed-in a hash (${hash}) that did NOT match our calculated hash(${newHash}). Rejecting.`, newTransaction )
        ...
        if( src.startsWith('_') ) { //a)
            ...
            if( !this.walletVerify(publicKey, txSig, newHash) ){
                if( this.outputDebug ) console.log( ` xx Signature was INVALID, rejecting transaction.`)
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( data.reverseAuth ){
            ...
            if( src.startsWith('_') )
                ...
                // we need a seq that is valid and +1 on tx or blockSeq (depending on if trans is being done for writing block or just mempool)
                // SIGNING REQUIRED (seq NEEDED!) -- TODO should queue it up and look later (ex. if out of order), for blockSeq know that trans related to a block?
                if( !( seq === (srcWallet.tx.seq + 1) || seq === (srcWallet.blockSeq.seq + 1) ) ){ 
                    return { error: `X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} invalid: either seq = srcWallet.tx.seq(${srcWallet.tx.seq})+1 or blockSeq.seq(${srcWallet.blockSeq.seq})+1... missing transactions? SKIPPING.`, 
                            seq: srcWallet.tx.seq, balance: Number(srcWallet.balance) }
                }
            } 
            ...
            if(  Number(srcWallet.balance) < (amount+fee) && src !== '_' ){
        ...
        // ---------------------- 
        let balance, result
        balance = fixRounding( Number(srcWallet.balance || 0) - (amount+fee) )
        result = this.updateWallet( src, { balance, tx: { amount: - (amount+fee), seq, hash } }) // updated seq saved
        if( result.error ) return result

        const destWallet = this.getWallet(dest)        
        balance = fixRounding( Number(destWallet.balance || 0) + amount )
        result = this.updateWallet( dest, { balance, tx: { amount, hash } })
        if( result.error ) return result
        ...
        if( fee !== 0 ){
            ...
            result = this.updateWallet( '_mint', { balance, tx: { amount: fee, hash } })
            if( result.error ) return result
        }
    ...
    walletSnapshots( wallets ){
        ...
        for( const name of wallets ){
            ...
            this.snapshots[publicKey] = this.wallets[publicKey] || {}
            snapCnt++
        }
    ...
    walletRestores( wallets ){
        ...
        for( const name of wallets ){
            ...
            this.wallets[publicKey] = this.snapshots[publicKey]
            restoreCnt++
        }
    ...
    walletBalances(names = [], compact = false ) {
        ...
        if( compact ){
            return publicKeys.map( publicKey => `${this.wallets[publicKey].name}: $${this.wallets[publicKey].balance}` ).join(',')
            ...
            wallets.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
                ...
                if( !(names.length===0 && i.name === '_') && i.balance>0 ){
                    const seqInfo = !i.blockSeq?.seq || i.blockSeq?.seq === 0 ? ':    ' : '/' + i.blockSeq.seq + ':' + ' '.repeat(3 - i.blockSeq.seq.toString().length)
