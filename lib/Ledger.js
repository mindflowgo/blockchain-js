...
export default class Ledger {
    ...
    transactionSign({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( hash !== '' && hash !== newHash ){
            console.log( ` [transactionSign] Transaction passed-in a hash (${hash}) that did NOT match our calculated hash(${newHash}). Rejecting.`, newTransaction )
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( data.reverseAuth ){
            ...
            else if( seq > 0 && seq !== srcWallet.tx.seq + 1 ){ // SIGNING REQUIRED (seq NEEDED!) -- TODO should queue it up and look later (ex. if out of order)
                // console.log( `  X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} : ERROR expected next seq(${srcWallet.tx.seq+1})... missing transactions? SKIPPING.` )
                return { error: `X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} invalid: EXPECTING next seq(${srcWallet.tx.seq+1})... missing transactions? SKIPPING.`, 
                        seq: srcWallet.tx.seq, balance: Number(srcWallet.balance) }
            } else if( seq < 1 ) seq = srcWallet.tx.seq + 1
        ...
        let result = this.updateWallet( src, { balance, tx: { amount: - (amount+fee), seq, hash } }) // updated seq saved
    ...
    walletBalances(names = []) {
        ...
        // BUGBUG TODO display is for debugging
