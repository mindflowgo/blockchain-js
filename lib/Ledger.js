...
export default class Ledger {
    ...
    updateWallet(name, walletData={}) {
        ...
        // upon creation need seq/balance; and of course publicKey cannot be overwritten
        const wallet = { ...this.wallets[publicKey] || {name, seq: 0, balance: 0}, ...walletData, publicKey }
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( data.reverseAuth ){
            ...
            signedTransaction = { timestamp: time(), src, dest, amount, fee, type: `reversal:${type}`, source: hash,  ...data }

        } else {
            // get users current sequence #, anticipate signing with this + 1
            seq = this.getTransactionSeq( srcWallet.publicKey )

            // go through motion of signing transaction to see if it would pass (otherwise no point proceeding)
            signedTransaction = this.transactionSign({src, dest, amount, fee, type, seq: seq+1, txSig, hash, ...data})
        ...
        // transaction SUCCEEDED, increment seq now!
        this.getTransactionSeq(srcWallet.publicKey, 1)
