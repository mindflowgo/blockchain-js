...
export default class Ledger {
        ...
        for( const walletAddress of walletAddresses ){
            ...
            if( blockIdx > -1 ) {
                ...
                if( !reverse ){
                    ...
                    if( idx===0 && txUpdate ) tx.seq = Math.max(tx.seq, onChain.seq)
                } else {
                    ...
                    if( idx===0 && txUpdate ) tx.seq = Math.max(tx.seq - 1, 0, onChain.seq)
                }
    ...
    walletBalances(names = [], compact = false ) {
        ...
        if( compact ){
            ...
            debug('dim',`   = Name =${' '.repeat(14)} = TX Balance =${' '.repeat(9)} = Block Balance =${' '.repeat(6)} = In Blocks =`)
            wallets.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
                ...
                if( !(names.length===0 && name === '_') && i.tx.balance>0 ){
                    ...
                    debug('dim',`   - ${name}${seqInfo}${' '.repeat(20-(seqInfo.length+name.length))} $ ${i.tx.balance || '0'} `
                               +`${' '.repeat(20-(i.tx.balance||'0').toString().length)} $ ${onChainBal} ${' '.repeat(20-(onChainBal).toString().length)} ${i.onChain.historyIdx || ''}`)
