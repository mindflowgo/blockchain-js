...
export default class Ledger {
    ...
    createWallet(name) {
        if( !name || name.startsWith('_') || name.indexOf(':')>0 ) {
            debug('red',`you need to give a name for the wallet (cannot *start* with _; cannot have colon in it)`)
            return
        }
        ...
        if( !userWallet.privateKey ){
            debug('dim',` - Updating/creating (${name}) with new public/privateKey`)
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}, options = {}){
        const { blockIdx = -1, blockIsNew = false, txUpdate = false, testOnly= false, reverse = false }= options
        ...
        if( this.debugOutputLevel ){
            debug('cyan',`  ~ #${blockIdx}:transaction${reverse?' *REVERSED*':''} (${src.split(':')[0]}${seq>0 ?'/'+seq:''}${srcWallet.privateKey?'[Signed]':''})`
                        +` -> (${dest.split(':')[0]||dest}) $${amount} `+( fee ? `fee(${fee})`  : '') + ` type(${newTransaction.type || ''}) ` + (txUpdate ? '[txUpdate]' : '' ) 
    ...
    transactionWalletUpdate({ src, dest, amount, fee, seq, blockIdx = -1, txUpdate = false, reverse = false }) {
        ...
        for( const walletAddress of walletAddresses ){
            ...
            if( blockIdx > -1 ) { // if block, we update onChain values only
                ...
                if( idx===0 ) onChain.seq = !reverse ? (seq ?? 0) : (onChain.seq > 0 ? onChain.seq - 1 : 0)
                if( txUpdate ){ // mirror onto the tx settings
                    tx.amount = walletAmount[idx]
                    tx.balance = fixRounding( tx.balance + tx.amount )
                    if( idx===0 ) tx.seq = !reverse ? Math.max(tx.seq, onChain.seq) : (tx.seq > 0 ? tx.seq - 1 : 0)
                }
            } else {
                tx.amount = walletAmount[idx]
                tx.balance = fixRounding( wallet.tx.balance + tx.amount )
                if( idx===0 ) tx.seq = !reverse ? (seq ?? 0) : (tx.seq > 0 ? tx.seq - 1 : 0)
            }
        ...
        if( compact ){
            ...
            wallets.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
                ...
                if( !(names.length===0 && name === '_') && i.tx.balance>0 ){
                    ...
                    debug('dim',`   - ${name}${seqInfo}${' '.repeat(20-(seqInfo.length+name.length))} $ ${i.tx.balance || '0'} ${' '.repeat(20-(i.tx.balance||'0').toString().length)} $ ${i.onChain.balance || '0'}`)
