...
export default class Ledger {
    ...
    reset(){
        ...
        addresses.forEach( publicKey => {
            ...
            this.wallets[publicKey].onChain = { seq: 0, amount: 0, balance: 0, historyIdx: [] } // levels at
            this.wallets[publicKey].tx = { seq: 0, amount: 0, balance: 0 }
        })
    }
    ...
    transactionSign({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        ...
        if( hash !== '' && hash !== newHash ){
            if( this.debugOutputLevel ) console.log( ` [transactionSign] Transaction passed-in a hash (${hash}) that did NOT match our calculated hash(${newHash}). Rejecting.`, newTransaction )
        ...
        if( src.startsWith('_') ) { //a)
            ...
            if( !this.walletVerify(publicKey, txSig, newHash) ){
                if( this.debugOutputLevel ) console.log( ` xx Signature was INVALID, rejecting transaction.`)
    ...
    transaction({src, dest, amount, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}, options = {}){
        const { blockIdx = -1, blockIsNew = false, txUpdate = false }= options
        ...
        if( data.reverseAuth ){
            ...
            if( src.startsWith('_') )
                ...
                if( blockIdx === -1 && seq !== (srcWallet.tx.seq + 1) )
                    return { error: `X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} invalid: seq = srcWallet.tx.seq(${srcWallet.tx.seq})+1... missing transactions?`, 
                            seq: srcWallet.tx.seq, balance: Number(srcWallet.tx.balance) }
                else if( blockIdx > -1 && !blockIsNew && seq !== (srcWallet.onChain.seq + 1) )
                    return { error: `X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} invalid: block#${blockIdx} should be onChain.seq(${srcWallet.onChain.seq})+1... missing transactions?`, 
                            seq: srcWallet.tx.seq, balance: Number(srcWallet.onChain.balance) }
                // blockIdx & blockIsNew means we're running an audit before inserting block, so onChain = seq-1
                else if( blockIdx > -1 && blockIsNew && seq < (srcWallet.onChain.seq - 1) )
                    return { error: `X transaction (${srcWallet.name}/${seq}) -> (${dest.split(':')[0]||dest}) $${amount} FAILED audit: block#${blockIdx} should be onChain.seq(${srcWallet.onChain.seq})-1...`, 
                            seq: srcWallet.tx.seq, onChainSeq: srcWallet.onChain.seq, balance: Number(srcWallet.onChain.balance) }
            } 
            ...
            if( src === '_' ) {
                ...
                // console.log( `Known ${src} balance(${srcWallet.tx.balance}) less than transaction amount(${amount}+fee=${amount+fee}). Rejecting.`)
        ...
        this.transactionWalletUpdate({ src, dest, amount, fee, seq, blockIdx, txUpdate })

        if( this.debugOutputLevel ){
            debug('cyan',`  ~ transaction${data.reverseAuth?' *REVERSED*':''} (${src.split(':')[0]}${seq>0 ?'/'+seq:''}${srcWallet.privateKey?'[Signed]':''})`
                        +` -> (${dest.split(':')[0]||dest}) $${amount} `+( fee ? `fee(${fee})`  : '') + ` type(${newTransaction.type || ''}) blockIdx(${blockIdx})` )
    ...
    transactionWalletUpdate({ src, dest, amount, fee, seq, blockIdx = -1, txUpdate = false }) {
        // update ledger balances
        // ---------------------- 
        let balance, result, tx, onChain

        debug(this.debugOutputLevel ? 'green' : 'dim', `        [transactionWalletUpdate] (${src.split(':')[0]}) -> ${dest.split(':')[0]} $${amount} seq(${seq}) blockIdx(${blockIdx}) txUpdate(${txUpdate})`)
        ...
        for( const walletAddress of walletAddresses ){
            ...
            if( blockIdx > -1 ) { // if block, we update onChain values only
                // txUpdate = src.startsWith('_') || seq > tx.seq 
                // || blockIdx === 0
                // seq = blockIdx // hack: set seq to blockIdx for admin accounts to trigger txUpdate
                onChain.amount = walletAmount[idx]
                onChain.balance = fixRounding( onChain.balance + onChain.amount )
                if( idx===0 ) onChain.seq = seq ?? 0
                if( txUpdate ){ // mirror onto the tx settings
                    tx.amount = walletAmount[idx]
                    tx.balance = fixRounding( tx.balance + tx.amount )
                    if( idx===0 ) tx.seq = Math.max(tx.seq, onChain.seq)
                }
            } else {
                tx.amount = walletAmount[idx]
                tx.balance = fixRounding( wallet.tx.balance + tx.amount )
                if( idx===0 ) tx.seq = seq ?? 0
            }
    ...
    transactionReverse({src, dest, amount, fee, txUpdate, ...data }){
        ...
        return this.transaction({src, dest, amount: -amount, fee: -fee, ...data, hash: '', txUpdate, reverseAuth:true })
    }
    ...
    walletBalances(names = [], compact = false ) {
        ...
        if( compact ){
            ...
            debug('dim',`   = Name =${' '.repeat(19)} = TX Balance =${' '.repeat(4)} = Block Balance =`)
            wallets.filter( w => w.name ).sort((a, b) =>  a.name.localeCompare(b.name)).forEach( i=>{
                ...
                if( !(names.length===0 && name === '_') && i.tx.balance>0 ){
                    ...
                    seqInfo = !seqInfo ? ':         ' : '/' + seqInfo + ':' + ' '.repeat(8 - seqInfo.length)
                    debug('dim',`   - ${name}${seqInfo}${' '.repeat(15-name.length)} $ ${i.tx.balance || '0'} ${' '.repeat(15-i.tx.balance.toString().length)} $ ${i.onChain.balance || '0'}`)
                    
                }
                balances[i.name] = Number(i.tx.balance)
                })
        }
        // don't pass back privateKey but all other info from the wallet is ok
        return wallets.map( ({ privateKey, ...data }) => data )
    }
