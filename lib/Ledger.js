...
export default class Ledger {
    ...
    createWallet(name) {
        if( !name || name.startsWith('_') || name.indexOf(':')>0 ) {
            debug('red',`You need to give a name for the wallet (cannot *start* with (_); cannot have a colon(:) in it)`)
            return
        }
    ...
    walletBalances(names = [], compact = false ) {
        ...
        if( compact ){
            ...
            debug('dim',`   = Name =${' '.repeat(14)} = TX Balance =${' '.repeat(9)} = Block Balance =`)
        ...
        // don't pass out privateKey EVERY - but all other info from the wallet is ok
