...
export default class Miner {
    ...
    static start({ nodeName, host, port, hosts, dataPath }) {
        ...
        this.P2P = new P2P( this.nodeName, host, port, hosts, this.Blockchain, ()=>{return this.Mempool.Queue.length()} )
        ...
        this.worker.node.on('message', ({ action, ...result }) => {
            switch( action ){
                ...
                case 'UNSOLVED':
                    ...
                    break    
                case 'SOLVED':
                    this.worker.blockData = result.blockData
    ...
    static startMining() {
        ...
        switch( this.worker.status ){
            case 'READY': {
                ...
                mineTransactions = this.Mempool.Queue.getMinerSorted({ miner: this.nodeName, maxTransactions: this.Blockchain.BLOCK_MAX_TRANSACTIONS })
                ...
                const blockData = this.Blockchain.prepareBlockData([ ...mineTransactions, ...rewardTransactions ])
                ...
                this.worker.block = {}
                this.worker.node.postMessage({action: 'MINE', block: blockData, difficulty: this.Blockchain.difficulty})
                break
                }
            case 'MINING':
            case 'MINING_PAUSE':
            ...
            case 'SOLVED': {
                ...
                const addResult = this.Blockchain.addBlock(this.worker.blockData)
                ...
                debug('green',`SOLVED (in ${this.worker.blockData.powTime}s) block: transactionCnt(${transactionCnt}) (hashes: ${hashes?.length}, resetLedger->`
        ...
        setInterval(() => {
            if (this.P2P.nodeState === 'ONLINE' && (this.worker.status !== 'READY' || this.Mempool.Queue.length('miner',this.nodeName)) ){
                workerMineQueued()
            }
