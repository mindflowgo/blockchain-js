...
export default class Miner {
    ...
    pingResponseProcess( response ){
        ...
        for( let node of response.result ){
            ...
            if( node.error ){
                ...
                delete node.error
                this.peers[peerIdx] = { ...this.peers[peerIdx], ... node, pingNext: time() + NEXT_PING, pingError: 0 }
            }
        }
    }
    ...
    async syncMissingBlocks(){
        ...
        console.log( `   .. [syncMissingBlocks] connected peers: ` + this.peers.map( node => `${node.nodeName} [${node.blockchainHeight}] (${node.pendingTransactions?.length || 0})` ).join('; ') )
        const selNode = this.peers.reduce((max, item) => item.blockchainHeight > max.blockchainHeight ? item : max)

        if( selNode.blockchainHeight <= this.blockchain.height() )
            return { error: false }  // same height, nothing more to do.

        // not yet updated info
        if( !selNode.blockAtHeight?.index ) return { error: false }
        ...
        const requests = broadcastPeers.map(async (peer, idx) => {
            ...
            try {
                ...
                if( response.peers ){
                    ...
                    this.broadcastPeers({ path: '/node/announce', peers: newPeers, data: this.pingInfo(this.blockchain.height()) })
                        .then( response => this.pingResponseProcess(response) )
                }
    ...
    mine() {
        ...
        switch( this.workerStatus ){
            ...
            case 'SOLVED': {
                ...
                const { hashes, newHashes, addBlockCnt, transactionCnt }= addResult
