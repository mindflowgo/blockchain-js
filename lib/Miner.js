 ...
 * PUBLIC METHODS
 * 
 * - blockchainLedgerBalances
 * 
 * - transactionFee: { amount, fee=0 }
 * 
 * - transaction: {src, dest, amount, fee, seq, txSig, hash, ...}
 * 
    ...
    mine() {
        ...
        switch( this.workerStatus ){
            case 'READY':
                ...
                const { block } = this.blockchain.addBlock(blockData, { prepareOnly: true })
            ...
            case 'SOLVED':
                ...
                if( result.block ){
                    console.log( `[/blocks/announce]`, result.block )
