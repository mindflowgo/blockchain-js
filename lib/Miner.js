...
export default class Miner {
    ...
    async minerAnnounceResponse( response ){
        ...
        const latestBlock = this.blockchain.getBlock() // latest block
        ...
        switch( this.workerStatus ){
            case 'READY':
                ...
                if( rewardTransactions.filter( t => t.error ).length > 0 ){
                    ...
                    this.workerStatus = 'UNDO_REWARDS'
                    break
                }
                ...
                newBlock = this.blockchain.addBlock(blockData, { prepareOnly: true })
                if( newBlock.error ){
                    this.workStatus = 'UNDO_BLOCK'
                    break
                }
            ...
            case 'SOLVED':
                newBlock = this.blockchain.addBlock(this.workerBlock)
                if( newBlock.error ){
                    this.workerStatus = 'UNDO_BLOCK'
                    break
                }

                // all pending that were solved in block are cleared
                const clearedHashes = this.pruneTransactions( newBlock.block.transactions )
                console.log( `SOLVED (in ${newBlock.block.powTime}s) block: clearing matching pendingTransactions (${clearedHashes.length})` )

                // tell everyone about block
                this.broadcastPeers({ path: '/blocks/announce', data: [newBlock.block], all: true })
            ...
            case 'UNDO_REWARDS':
                // only need to undo rewards as no block mining ever happened, (exclude those that errored out)
                mineTransactions = rewardTransactions.filter( t => !t.error )

            case 'UNDO_BLOCK':
                // lets undo the list of mined transactions as the mining failed...
                ...
                mineTransactions.forEach( t => {
                    this.rejectedTransactions.push( this.ledger.transactionReverse( t ) )
                })
