...
export default class Miner {
    ...
    heartbeat(){
        ...
        setInterval(async () => {
            ...
            if (peers.length > 0) {
                ...
                const staleHashes = this.transactionManager.deletePending({ timestamp: time()-PENDING_TRANSACTION_STALE, reverse: true })
                if( staleHashes )
    ...
    async syncMissingBlocks(){
        ...
        const response = await urlCall({ hostname: selNode.hostname, path: `/blocks?fromIndex=${fromIndex}&type=hashes`, nodeToken: this.nodeName })
        ...
        for( let i=this.blockchain.height()-1; i >= fromIndex; i-- ){
            if( !response.result[i-fromIndex] ) return { error: `Invalid /blocks result: `, result }
        ...
        try {
            ...
            if( newBlocks.length>0 ) debug('dim',`  + got blocks to add: ` + newBlocks.map( b => b.index ).join(',') )
            ...
            const foundHashes = this.transactionManager.deletePending({ hashes: addResult.hashes })
    ...
    startMining() {
        ...
        switch( this.worker.status ){
            case 'READY': {
                ...
                mineTransactions = this.transactionManager.getPending({ txStakeLabel: this.nodeName, maxTransactions: this.blockchain.transactionLimit.max })
                ...
                mineTransactions.forEach( t => {
                    if( t.fee>0 ) 
                        rewardTransactions.push( this.transactionManager.new(
            ...
            case 'CLEANUP': {
                ...
                if( this.worker.block?.transactions?.length > 0 ){
                    ...
                    const { error, pending, deleted }= this.transactionManager.reverseBatch(this.worker.block.transactions)
                    if( error ) debug('red', error)
                    debug('cyan',`    \ ${pending} transactions reset to pendingTransactions; ${deleted} transactions (mining fees, rewards?) fully deleted. No residual effect from the mined block.`)

                } else {
                    const { error, pending, deleted }= this.transactionManager.reverseBatch(rewardTransactions)
                    if( error ) debug('red', error)
                    debug('cyan',`    \ rewardTransactions: ${pending} transactions reset to pendingTransactions; ${deleted} transactions (mining fees, rewards?) fully deleted. No residual effect from the mined block.`)
                }
        ...
        setInterval(() => {
            if (this.worker.status !== 'READY' || this.transactionManager.pendingCnt(this.nodeName) ){
                workerMinePending()
            }
