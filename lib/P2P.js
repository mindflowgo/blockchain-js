...
import { urlCall, time, debug } from './helper.js'
...
export default class P2P {
    ...
    static start( nodeName, host, port, hosts ) {
    ...
    static getNodeState(state) {
        return this.nodeState
    }

    static setNodeState(state) {
        if (this.nodeState !== 'PREPARING') this.nodeState = state
    }

    // static nodeOffline(state) {
    //     if (this.nodeState !== 'PREPARING') this.nodeState = state
    // }
    ...
    static async heartbeat() {
        ...
        if (peers.length > 0) {
            ...
            // force sync every 10 heratbeats
            Mempool.clearStale( this.heartbeatCnt%10 === 0 )
        }
    }
    ...
    static pingInfo( queryBlockIndex=Blockchain.height()-1 ){
        ...
        const response = {
            ...
            pendingTransactionsCnt: Mempool.queueLen(),
    ...
    static async broadcastPeers({ path, data = '', hosts }) {
        ...
        const requests = broadcastPeers.map(async (host) => {
            const response = await callPeer(host, path, data) 
            ...
            if (response.peers) {
                ...
                if (newHosts.length > 0)
                    this.broadcastPeers({ path: '/node/announce', hosts: newHosts, data })
                        .then(response => this.pingResponseProcess(response))
            }
            return response
        })
    ...
    static async callPeer(hostname, path, data = null) {
        const request = { hostname, path, nodeToken: this.nodeName }
        if (data) request.body = data
        return await urlCall(request)
    }

    // Broadcast blocks to peers
    static broadcastBlocks(blocks) {
        return this.broadcastPeers({ path: '/blocks/announce', data: blocks })
    }

    // Broadcast blocks to peers
    static broadcastTransactions(transactions) {
        return this.broadcastPeers({ path: '/transactions/announce', data: transactions })
    }

    // scan through peers and see if we should ask for blocks from anyone
    static findPeerMostBlocks(){
        const peers = this.getPeersData()
        const myHeight = Blockchain.height()
        const latestBlock = Blockchain.getBlock()
        ...
        debug( 'bold', ` x US (${this.nodeName}) vs THEM (${selNode.nodeName}): blocks(${Blockchain.height()} vs ${selNode.blockchainHeight}) timestamp(${latestBlock.timestamp} vs ${selNode.blockAtHeight?.timestamp || 'n/a'}) (ours: #${latestBlock.index}/${latestBlock.timestamp}, theirs: #${selNode.blockAtHeight?.index || 'n/a'}/${selNode.blockAtHeight?.timestamp || 'n/a'}, finding last common node, and overwriting rest` )
    ...
    async syncPeerBlocks(hostname){
        ...
        const fromIndex = Math.max(0, Blockchain.height()-100 )
        ...
        for( let i=Blockchain.height()-1; i >= fromIndex; i-- ){
            if( !response.result[i-fromIndex] ) return { error: `Invalid /blocks result: `, result: response.result }

            if( Blockchain.getBlock(i).hash === response.result[i-fromIndex].hash ){
                index = i
        ...
        try {
            ...
            if( newBlocks.length>0 ){
                ...
                const addResult = Blockchain.addChain(newBlocks, { forceOverwrite: true, txUpdate: true })
                if( addResult.error ) return addResult
                // if any of these are in pending, remove them
                foundHashes = Mempool.deleteBatch({ hashes: addResult.hashes })
