...
const BASE_TOKEN = '$'                  // any token *MUST* start with $ as this is system identifier for tokens
...
export default class TransactionHandler {
    ...
    // user starting with '_' are admin users, '$' is prefix for ecosystem token type
    ...
    static buildGenesisTransactions(supply = GENESIS_ISSUE, token){
        ...
        debug(`[buildGenesisTransactions] token(${token}) nodeName(${this.nodeName}) )`)
        if( !token || token.substring(0,1) !== '$' ) token = BASE_TOKEN
        const airdropAuth = Wallet.buildNameWithPublicKey(this.nodeName)
        const transactionData = { src: '$_', dest: token, amount: supply, token, type: 'mintIssue', 
                                  airdropAuth, timestamp: token === TransactionHandler.baseToken ? 0 : time() }
        const genesisTransaction = this.prepareTransaction(transactionData)
        if( genesisTransaction.error ) return genesisTransaction
        // returning genesis gransactions initialized
        return [ genesisTransaction ]
    }

    static getFee({ amount, token, fee=0 }){
    ...
    static auditTransactions(blockMiner, blockIdx, transactions, expectedMiningReward) {
        ...
        for( const transaction of transactions ){
            ...
            const newTransaction = this.prepareTransaction(transaction,{ blockIdx: block.index, txUpdate: true })
            if( newTransaction.error ) transactionError = newTransaction.error
            const balanceCheck = this.checkTokenBalances(newTransaction,{ blockIdx: block.index })
            if( balanceCheck.error ) transactionError = balanceCheck.error
    ...
    static filter({ transactions, hashes = [], types = [] }) {
        ...
        if( types.includes('tokenIssue') ) typeList = 'mintIssue'
    ...
    static transactionSign({src, dest, amount, token = BASE_TOKEN, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}){
        // build transaction
        fee = fixRounding( Number(fee || 0) )
        amount = fixRounding( Number(amount || 0) )

        const newTransaction = { timestamp: time(), src, dest, amount, token, fee, type, seq, ...data } // exclude txSig, hash, meta from transaction 
    ...
    static prepareTransaction({src, dest, amount, token = BASE_TOKEN, fee = 0, type = '', seq = 0, txSig = '', hash = '', meta, ...data}, options = {}){
        ...
        if( reverse ){
            ...
            if( src.startsWith('$') )
                ...
                seq = srcWallet[token].tx.seq + 1 // auto-gen it as it doesn't need signed trans with seq (ex admins)
            else {
                ...
                // debug( 'green', `[ledger::transaction] (${srcWallet.name}/${seq}) blockIdx(${blockIdx}) trans-seq(${seq}) tx.seq(${srcWallet[token].tx.seq}) onChain.seq(${srcWallet[token].onChain.seq}) `)
                console.log(`  [TransactionHandler] token(${token}) srcWallet: `, srcWallet )
                if( blockIdx === -1 && seq !== (srcWallet[token].tx.seq + 1) )
                    return { error: `X transaction [${srcWallet.name}/${seq} -> ${dest.split(':')[0]||dest} ${token.substring(1)}$${amount} rejected - need seq = srcWallet[token].tx.seq+1(${srcWallet[token].tx.seq+1})...`, 
                            blockIdx, seq: srcWallet[token].tx.seq, balance: Number(srcWallet[token].tx.balance) }
                else if( blockIdx > -1 && seq !== (srcWallet[token].onChain.seq + 1) )
                    return { error: `X transaction [${srcWallet.name}/${seq} -> ${dest.split(':')[0]||dest} ${token.substring(1)}$${amount}] rejected - block#${blockIdx}; need seq = onChain.seq+1(${srcWallet[token].onChain.seq+1})..`, 
                            blockIdx, seq: srcWallet[token].onChain.seq, balance: Number(srcWallet[token].onChain.balance) }
            } 
    ...
    static checkTokenBalances({src, dest, amount, token, fee = 0, type = '', seq = 0, txSig = '', hash = '', meta, ...data}, options = {}){
        const { blockIdx = -1 }= options

        const srcWallet = Wallet.getUser(src)

        if( src === '$_' ) {
            debug('dim',`    ~ System Token Issue Transaction for: $${amount}`)

        } else if( blockIdx === -1 ){ // memory-only so far
            if( token === this.baseToken && Number(srcWallet[token].tx.balance) < (amount+fee) ){
                return { error: `${src.split(':')[0]} balance(${srcWallet[token].tx.balance}) less than transaction amount(${amount}+fee=${amount+fee}). Rejecting.`, balance: Number(srcWallet[token].tx.balance) }
            } else if( token !== this.baseToken && (Number(srcWallet[token].tx.balance) < amount || Number(srcWallet[this.baseToken].tx.balance) < fee)) {
                return { error: `${src.split(':')[0]} balance(${srcWallet[token].tx.balance}) less than transaction amount(${amount} or fee=${fee} more than fee-account(${srcWallet[this.baseToken].tx.balance}). Rejecting.`, balance: Number(srcWallet[token].tx.balance) }
            }
        } else if( blockIdx > -1 ){
            if( token === this.baseToken && Number(srcWallet[token].onChain.balance) < (amount+fee) ){
                return { error: `${src.split(':')[0]} onChain balance(${srcWallet[token].onChain.balance}) less than transaction amount(${amount}+fee=${amount+fee}). Rejecting.`, balance: Number(srcWallet[token].onChain.balance) }
            } else if( token !== this.baseToken && (Number(srcWallet[token].onChain.balance) < amount || Number(srcWallet[this.baseToken].onChain.balance) < fee)) {
                return { error: `${src.split(':')[0]} balance(${srcWallet[token].onChain.balance}) less than transaction amount(${amount} or fee=${fee} more than fee-account(${srcWallet[this.baseToken].onChain.balance}). Rejecting.`, balance: Number(srcWallet[token].onChain.balance) }
            }
        }
        ...
        if( this.debugOutputLevel ){
            debug('cyan',`  ~ #${blockIdx}:transaction${reverse?' *REVERSED*':''} [${src.split(':')[0]}${seq>0 ?'/'+seq:''}${srcWallet.privateKey?'[Signed]':''}`
                        ...
                        + (seq>0 || srcWallet[token].tx.seq>0 || srcWallet[token].onChain.seq>0 ? ` seq(${seq}) tx.seq(${srcWallet[token].tx.seq}) onChain.seq(${srcWallet[token].onChain.seq})` : '') )
    ...
    static walletTransactionData({ src, dest, amount, token = BASE_TOKEN, fee, seq, blockIdx = -1, txUpdate = false, reverse = false }) {
        ...
        const walletAmount = [-amount, amount, -fee, fee]
        const walletAddresses = fee !== 0 ? [src, dest, src, token] : [src, dest]
        // console.log( `[walletAddresses] (token(${token}) src(${src}) dest(${dest})): `, walletAddresses )
        let idx=0
        for( const walletAddress of walletAddresses ){
            if( idx>1 ) token = this.baseToken // fee/-fee always in baseToken
            const wallet = Wallet.getUser(walletAddress)

            tx = wallet[token].tx || { seq: 0, amount: 0, balance: 0 }
            onChain = wallet[token].onChain || { seq: 0, amount: 0, balance: 0, historyIdx: [] }
            ...
            if( blockIdx > -1 ) {
                ...
                tx.amount = walletAmount[idx]
                tx.balance = fixRounding( wallet[token].tx.balance + tx.amount )
                if( !reverse ){
            ...
            result = Wallet.update( walletAddress, { [token]:{ onChain, tx } })
    ...
    // prepareTransaction + Memool.addQueue + walletTransaction
    static new({src, dest, amount, token = BASE_TOKEN, fee = 0, seq = 0, txSig = '', hash = '', ...data}, options = {} ) {
        ...
        if( isNew ){
            // system users ($) no fees, others pay fees
            fee = src.startsWith('$') ? 0 : this.getFee({ amount, token, fee })

            const transactionData = {src, dest, amount, token, fee, seq, txSig, hash, ...data}
            transaction = this.prepareTransaction(transactionData,{ blockIdx, txUpdate, testOnly })
            ...
            if( transaction.error ) {
                ...
                transaction = { error: balanceCheck.error, index: blockIdx }
            ...
            if( !testOnly ){
                ...
                console.log( `Wallet Addreses (before)`, JSON.stringify(Wallet.addresses))
                const result = this.walletTransactionData({ src, dest, amount, token, fee, seq, blockIdx, txUpdate: true })
                // console.log( `walletTransactionData`, Wallet.addresses)
                console.log( `Wallet Addreses (after)`, JSON.stringify(Wallet.addresses))
                if( result.error ) return result
            }
        } else {
    ...
    static updateMeta(transactions,field,value){
        console.log( `[updateMeta] transactions:`, transactions )
        for( const t of transactions ){
            if( !t.meta ) t.meta = {}
    ...
    static reverse({src, dest, amount, token, fee, seq, hash, ...data}){
        ...
        const transaction = this.prepareTransaction(transactionData, options)
