...
export default class TransactionHandler {
    ...
    walletTransactionData({ src, dest, amount, token, fee, seq, blockIdx = -1, txUpdate = false, reverse = false }) {
        ...
        if( !src || !dest || amount == 0 )
            return { error: `Missing parameter (src: ${src}, dest: ${dest}, amount: ${token}${amount}` }
    ...
    processTransaction({src, dest, amount, token = '', type, fee = 0, seq = 0, txAuth = '', hash, ...data}, options = {} ) {
        ...
        if( balanceCheck.error ) {
            ...
            debug(5, `~ [processTransaction] FAILED @ checkTokenBalances: ${balanceCheck.error}`)
            return balanceCheck
        }
        ...
        if( transaction.error ) {
            ...
            debug(5, `~ [processTransaction] FAILED @ addSeqAndSign: ${transaction.error}`)
            return transaction
        }
        ...
        for( const transaction of transactions ){
            ...
            if( resultTransaction.error ){
                if( this.Mempool.Hashes.findBlockIdx(hash) === -1 ){
                    ...
                    debug( 5, `   .. ${resultTransaction.error} (Failed transaction; deleting hash)`)
                    ...
                    debug(5,`  ..${resultTransaction.error}`)
                }
