...
export default class TransactionHandler {
    ...
    createToken(supply, token = this.BASE_TOKEN, admin = this.nodeName){
        ...
        const createTokenTransaction = this.getSeqAndSign(transactionData)
        if( createTokenTransaction.error ) return createTokenTransaction
    ...
    auditTransactions(blockMiner, blockIdx, transactions, expectedMiningReward) {
        ...
        debug('dim', `// [auditTransactions] snapshotting users (${transactionUsers.join(',')}) ------------` + JSON.stringify(this.Wallet.addresses))
        ...
        debug('dim', `\\ restored users (${transactionUsers.join(',')}) ------------ ` + JSON.stringify(this.Wallet.addresses))
    ...
    // getSeqAndSign + Memool.Queue.add + walletTransaction
    processTransaction({src, dest, amount, token = '', fee = 0, seq = 0, txSig = '', hash = '', ...data}, options = {} ) {
        ...
        if( transaction.error ) {
            ...
            debug('dim', ` ~ [processTransaction] FAILED @ getSeqAndSign: ${transaction.error}`)
            return transaction
        }
