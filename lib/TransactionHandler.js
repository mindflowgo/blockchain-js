...
const BLOCK_TRANSACTION_TYPES_ADMIN = ['tokenIssue','tokenAirDrop']
const BLOCK_TRANSACTION_TYPES_SYSTEM = ['miningReward', 'miningFees']
...
export default class TransactionHandler {
    ...
    setHelperClasses(Mempool, Wallet){
        this.Mempool = Mempool
        this.Wallet = Wallet
    }
    ...
    buildGenesisTransactions(supply = GENESIS_ISSUE, token){
        ...
        if( !token || !token.endsWith('$')) token = BASE_TOKEN
        const transactionData = { src: this.MINT_TOKEN, dest: token, amount: supply, token, type: 'tokenIssue', timestamp: 0 }
        if( token !== BASE_TOKEN ){
            transactionData.airdropAuth = this.Wallet.buildNameWithPublicKey(this.nodeName)
    ...
    auditTransactions(blockMiner, blockIdx, transactions, expectedMiningReward) {
        ...
        for( const transaction of transactions ){
            ...
            const newTransaction = this.getSeqAndSign(transaction,{ blockIdx, txUpdate: true })
            if( newTransaction.error ) transactionError = newTransaction.error
            const balanceCheck = this.checkTokenBalances(newTransaction,{ blockIdx })
            ...
            this.walletTransactionData({ ...newTransaction, blockIdx, txUpdate: true })
    ...
    filter({ transactions, hashes = [], types = [] }) {
        ...
        if( types.includes('tokenIssue') ) typeList = 'tokenIssue'
    ...
    getSeqAndSign({src, dest, amount, token, fee = 0, type = '', seq = 0, txSig = '', hash = '', ...data}, options = {}){
        ...
        let newTransaction
        // extract token from amount (if present that way)
        { [amount, token] = this.extractTokenFromAmount(amount, token) }
    ...
    walletTransaction({ name, token, amount, seq }, options = {}){
        const { blockIdx = -1, txUpdate = false, reverse = false }= options
        ...
        //     tx = wallet[token].tx || tokenDefaults[token].tx // { amount: 0, balance: 0 }
        // onChain = wallet[token].onChain || tokenDefaults[token].onChain // { amount: 0, balance: 0, historyIdx: [] }
        ...
        if( blockIdx > -1 ) {
            ...
            if( !reverse ){
                ...
                if( seq ){ // for transaction src, we manage seq
                    wallet.seq.onChain = seq
                    wallet.seq.tx = Math.max(wallet.seq.tx, wallet.seq.onChain)
                }
            } else {
                ...
                if( seq ){
                    wallet.seq.onChain = Math.max(wallet.seq.onChain - 1, 0)
                    wallet.seq.tx = wallet.seq.onChain; // Math.max(tx.seq - 1, onChain.seq, 0)
                }
            }
            ...
            if( txUpdate ){ // mirror onto the tx balance (ex. on chain rel)
                ...
                tokenTx.balance = fixRounding( tokenTx.balance + amount )
            }
        } else {
            tokenTx.amount = amount
            tokenTx.balance = fixRounding( tokenTx.balance + amount )
            if( !reverse ){
                if( seq ) wallet.seq.tx = Math.max(seq, wallet.seq.onChain)
            } else {
                if( seq ) wallet.seq.tx = Math.max(wallet.seq.tx - 1, wallet.seq.onChain, 0)
            }
        }
        // pop back prior values
        // wallet[token].tx = tokenTx
        // wallet[token].onChain = tokenOnChain
    ...
    rewardTransaction({src, dest, amount, token, type, blockIdx, ...data}) {
        // extract token from amount if present, ex. sol$100
        { [amount, token] = this.extractTokenFromAmount(amount, token) }

        const transactionData = {src, dest, amount, token, type, ...data}
        const transaction = this.getSeqAndSign(transactionData,{ blockIdx })
    ...
    processTransaction({src, dest, amount, token = '', fee = 0, seq = 0, txSig = '', hash = '', ...data}, options = {} ) {
        ...
        const isNew = !hash ? true : !this.Mempool.exists(hash)
...
// console.log( `[processTransaction] transactionData:`, transactionData)
            transaction = this.getSeqAndSign(transactionData,{ blockIdx, txUpdate, testOnly })
            ...
            // update wallet balances - if txUpdate true it will do wallet tx at same time as onChain
            const result = this.walletTransactionData({ ...transaction, blockIdx, txUpdate })
            ...
            transaction = { error: `Existing: [${this.Wallet.getNameOnly(src)}/${seq} -> ${this.Wallet.getNameOnly(dest)} [${token}]${amount}] but updated wallet.onChain & hashes-lookup: block = #${blockIdx}${data.seq > 0 ? `; onChain(${data.seq})` : '' }`, updateSeq: true, index: blockIdx }
            ...
            if( !testOnly ){
                this.Mempool.updateHashBlockIdx(hash,blockIdx)
                console.log( `[processTransaction] Wallet Addreses (before)`, JSON.stringify(this.Wallet.addresses))
                const result = this.walletTransactionData({ src, dest, amount, token, fee, seq, blockIdx, txUpdate: true })
                // console.log( `walletTransactionData`, this.Wallet.addresses)
                console.log( `[processTransaction] Wallet Addreses (after)`, JSON.stringify(this.Wallet.addresses))
                if( result.error ) return result
            }
        } else {
    ...
    removeMeta(transactions){
        console.log( `[removeMeta] transactions:`, JSON.stringify(transactions))
        transactions = transactions.map( ({meta, ...data})=> data )
        console.log( ` ~~ after transactions:`, JSON.stringify(transactions))
    }
