...
export default class TransactionHandler {
    ...
    tokenCreation(amount, token = this.BASE_TOKEN, admin){
        ...
        if( wallet.created < (time()-1) ){
            ...
            return { error: `Token already created, can't prepare genesis token issue (${token}).` }
        }
    ...
    transactionTypeOperations({ src, dest, type, amount, token, admin, ...data }) {
        if( type === 'tokenCreate' ) {
            const destWallet = this.Wallet.getUser(dest)
            if( destWallet.error ){
                debug( 1, `<red>Some REALLY wrong happened. Token issue transaction exists without a matching WALLET for the token (${dest}), type(${type})`)
                return destWallet
            }
    ...
    auditTransactions( blockMiner, blockIdx, transactions, expectedMiningReward ) {
        ...
        for( const transaction of transactions ){
            ...
            if( transaction.type === 'miningReward' ){
                ...
                transactionError = `Block audit failed: hash(${transaction.hash} txAuth(${transaction.txAuth}) type(${transaction.type}) INVALID`
            }
    ...
    transactionSign({src, dest, amount, token, fee = 0, type = '', seq = 0, txAuth = '', hash, meta, ...data}, signer){
        ...
        if( src === this.BASE_TOKEN && TRANSACTION_TYPES_SYSTEM.includes(type) ) { //a)
            ...
            debug( 'cyan', `   ~ ${src}/${type} -> signing exempt` )
            ...
            if( decodedHash !== newHash ){
                ...
                return { error: `[transactionSign] Transaction signature (for: ${signerName}) was INVALID, rejecting.` }
            }
            ...
            console.log( ` [transactionSign] txAuth(${txAuth})`)
            if( txAuth?.error ) return txAuth
        }
    ...
    checkTokenBalances({src, dest, amount, token = '', fee = 0, type = '', seq = 0, txAuth = '', hash = '', meta, ...data}, options = {}){
        ...
        if( src === this.MINT_TOKEN ) {
            ...
            return { error: false, balance: 1000000000000 }
        }
        ...
        if( srcWallet.error )
            if( srcName.endsWith('$') )
                return { error: `${srcName} token not available on node, create it? Rejecting.` }
            else
                return { error: `${srcName} account not known on node. Rejecting.` }
        else if( !srcWallet[token] )
            return { error: `${srcName} does not have any ${token}. Rejecting.` }
        ...
        if( fee > 0 && src.includes('$') ) {
            feeWallet = this.Wallet.getUser( srcWallet.admin )
        ...
        if( srcWallet[token].balance < 0 )
            return { error: `${srcName} balance(${token}${srcWallet[token].balance}) less than transaction amount(${token}${amount}+fee)=${token}${amount+fee}. Rejecting.`, balance: srcWallet[token].balance }
        else if( feeWallet[this.BASE_TOKEN].balance < 0 )
            return { error: `Insufficient balance for fees (${this.BASE_TOKEN}${fee} by ${feeWallet.name}.` }
        ...
        if( this.debugOutputLevel ){
            debug('cyan',`  ~ #${blockIdx}:transaction${reverse?' *REVERSED*':''} [${srcName}${seq>0 ?'/'+seq:''}${srcWallet.privateKey?'[Signed]':''}`
        ...
        return { error: false, balance: srcWallet[token].balance }
    }
    ...
    processTransaction({src, dest, amount, token = '', type, fee = 0, seq = 0, txAuth = '', hash, ...data}, options = {} ) {
        ...
        if( alreadyExists ) {
            if( blockIdx > -1 ) {
                ...
                if( manageMempool ){
                    const updateResult = this.Mempool.Hashes.updateBlockIdx(hash,blockIdx)
                    if( updateResult.error ) return updateResult
                    // delete queue entry 
                    const deleteResult = this.Mempool.Queue.delete( hash )
                ...
                debug('dim', `  ~ [processTransaction] ${log}`)
        ...
        if( balanceCheck.error ) {
            balanceCheck.index = blockIdx
            debug('dim', `  ~ [processTransaction] FAILED @ checkTokenBalances: ${balanceCheck.error}`)
            return balanceCheck
        }
        ...
        if( transaction.error ) {
            ...
            debug('dim', `  ~ [processTransaction] FAILED @ addSeqAndSign: ${transaction.error}`)
            return transaction
        }

        this.updateMeta( [transaction],'balance',balanceCheck.balance )
        ...
        if( blockIdx === -1 ){
            ...
            if( manageMempool ) {
                const addCnt = this.Mempool.Queue.add( transaction )
            ...
            if( manageMempool ) {
                const updateResult = this.Mempool.Hashes.updateBlockIdx(transaction.hash,blockIdx)
                if( updateResult.error ) return updateResult
            }
        }
    ...
    processTransactions( transactions, options = {} ){
        ...
        if( blockIdx>-1 ) debug('cyan',`\n- [block#${blockIdx}]: (transactions: ${transactions.length}) ------------------------------------------------------------------------` )
        for( const transaction of transactions ){
            const resultTransaction = this.processTransaction(transaction, options)
            const hash = transaction.hash || resultTransaction.hash
            hashes.push( hash )
            resultTransactions.push( resultTransaction ) // includ transaction.error results
            if( resultTransaction.error ){
                if( this.Mempool.Hashes.findBlockIdx(hash) === -1 ){
                    // failed so delete hash-look-up reference to this transaction hash
                    debug( 'dim', `   .. ${resultTransaction.error} (Failed transaction; deleting hash)`)
                    ...
                    debug('dim',`  ..${resultTransaction.error}`)
                }
                error += resultTransaction.error + ','
            } else {
                transactionCnt++
        ...
        return { error: error || false, transactions: resultTransactions, transactionCnt, hashes, newHashes, deleteHashes }
    }
    ...
    syncTxToChain( addresses, minerTokens = false ){
        ...
        if( minerTokens ){ 
            ...
            debug( 'cyan', `[TransactionHandler::syncTxToChain] tokens active on this node; will sync tx >> onChain`, addresses )
        } else {
