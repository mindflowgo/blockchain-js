...
// admin types not adjustable: tokenIssue, tokenAirDrop, miningReward, miningFees
...
export default class Blockchain {
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( !readOnly && (newBlock.minerName === this.nodeName || newBlock.minerName === 'genesis' ) ){
            this.TransactionHandler.removeMeta(newBlock.transactions)
console.log( `[addBlock] writing transactions without meta data: `, newBlock.transactions)
        ...
        if( adminCnt>0 ){
            const issueTransactions = this.TransactionHandler.filter({ transactions: newBlock.transactions, types: 'tokenIssue' })
            for( const transaction of issueTransactions ){
                const destWallet = this.Wallet.getUser(transaction.dest, false)
                if( destWallet.error && transaction.type === 'tokenIssue' ){
                ...
                this.Wallet.update(transaction.dest, { airdropAuth: transaction.airdropAuth })
            }
        }
