...
export default class Blockchain {
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( prepareOnly ){
            if( !blockData?.transactions || blockData.transactions.length < this.transactionLimit.min ) 
                return { error: `Need min ${this.transactionLimit.min} transactions per block; there are ${blockData.transactions.length} in this block. Rejecting.`, block: {} }
        ...
        if( readOnly && !newBlock.fileCache){
            ...
            return { error: `Read-Only mode BUT *NO* pre-existing could be loaded for index(${index}), aborting.`, block: {} }
        }
        ...
        const syncResult = this.transactionManager.newBatch( newBlock.transactions, { blockIdx: newBlock.index, txUpdate })
    ...
    auditBlockValid( block ){
        ...
        if (index > 0) {
            ...
            if( !block.hash.startsWith(hashPrefix) ){
                debug('red',`Block#${block.index}: Invalid hash (${block.hash}), first ${calc.difficulty}-bytes must be 0, rejecting!`, block)
    ...
    getBlockchain(fromIndex=0,size=0,fields='') {
        ...
        if( fields === 'hashes')
            ...
            // peel off meta-data in block before passing on all data
    ...
    addBlockchain(chain, blockOptions = {}){
        ...
        // if our chain is longer, we extract our minted transactions > pending; cut chain short, add external blocks
        ...
        if( forceOverwrite && this.height() > fromIndex ){
            ...
            for( const block of dropChain ){
                ...
                for( const transaction of block.transactions ){
                    if( !BLOCK_TRANSACTION_TYPES.includes(transaction.type) ) continue // only user-transactions
                    debug('dim',`    -> +re-saving to pending:    ${transaction.src.split(':')[0]}/${transaction.seq} -> ${transaction.dest.split(':')[0]} $${transaction.amount}`)
                    transaction.meta = { txStake: this.nodeName + ':', reAddTimestamp: time() }
        ...
        let pendingResult = this.transactionManager.reloadPending()
        if( pendingResult.error ) return pendingResult            
