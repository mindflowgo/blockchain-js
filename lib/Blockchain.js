...
export default class Blockchain {
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( readOnly && (prepareOnly || forceOverwrite) )
            return { error: `[addBlock] Cannot add a block with readOnly withprepareOnly or forceOverwrite. Rejecting.`}
        ...
        if( blockData?.minerName !== 'genesis' && (blockData?.index && blockData.index !== index) ){
            ...
            return { error: `[addBlock] Attempted adding new block to chain but it's index=${blockData.index}), accepting index=${index} now (or empty index). Rejecting.` }
        }
        ...
        const addValid = this.addValidate( newBlock )
        if( addValid.error ){
