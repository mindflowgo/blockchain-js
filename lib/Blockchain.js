...
export default class Blockchain {
    ...
    constructor( version, nodeName, dataPath, TransactionHandler, Mempool, Wallet) {
        ...
        while( 1 ){
            ...
            if( result.error ){
                ...
                if( result.block ) debug( 1, `<red>ERROR:</>\n` + result.error)
                break
            }
            // block = result.block
            loadBlockIndex++
        ...
        if( loadBlockIndex < 1 ){
            ...
            let addResult
            if( !genesisTransactions.error ) {
                const genesisBlockData = this.prepareBlockData(genesisTransactions, true)
                addResult = this.addBlock(genesisBlockData, {forceOverwrite: true, txUpdate: true})
                console.log( `addResult`, addResult)
            }

            if( genesisTransactions.error || addResult.error ){
                debug( 1, `<red>Error</> SERIOUS problem, unable to create genesis block. Check your filesystem and run again`)
                process.exit(-1)
            }
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( addValid.error ){
            debug( 1, `<red>ERROR:</> [addBlock::addValid] addValidate FAILED on #${index}: ${addValid.error}`)
        ...
        return { error: false, block: newBlock, hashes: syncResult.hashes, newHashes: syncResult.newHashes, 
    ...
    auditFullChainValid( chain = this.chain ) {
        ...
        for (let i = 0; i < chain.length; i++) {
            ...
            const result = this.verifyBlockValidToConnect( block )
            ...
            if( !block.isHashValid( calc.difficulty,block.hash ) || block.hash !== block.calcHash() ){
                ...
                return { error: `<red>Error</>: Invalid hash, first ${calc.difficulty}-bytes must be 0, rejecting!`, index }
            }        
    ...
    rollbackBlock( blockData ){
        // BUGBUG add code here
        debug( 1, `<red>OOPS</> ! Blockchain::rollbackBlock called, no code in place.`)
    }
}