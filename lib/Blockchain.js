...
export default class Blockchain {
    ...
    auditBlockValid( currentBlock ){
        ...
        if( currentBlock.index > 0 && !currentBlock.hash.startsWith(hashPrefix) ){
            console.log(`Block#${currentBlock.index}: Invalid hash (${currentBlock.hash}), first ${calc.difficulty}-bytes must be 0, rejecting!`)
    ...
    updateTransactionsFromBlock( blockIdx=0 ) {
        const addHash = ( t, blockIdx ) => {
            const transaction = this.ledger.transaction(t)
            if( transaction.error ){
                error += transaction.error + ','
            } else {
                transactionCnt++
                this.findOrCreateHash( transaction.hash, blockIdx )
            }
        }
        ...
        let error = ''
        if( blockIdx>0 ){
            this.getBlock(blockIdx).transactions.forEach( t => addHash(t,blockIdx) )
                
        } else {
            console.log( `\n[updateTransactionsFromBlock] Scanning all blocks & building...` )
            this.chain.forEach( block => {
                console.log( `- [block#${block.index}]: -----------------------------------` )
                block.transactions.forEach( t => addHash(t,block.index) )
            })
        }
        return { error: error || false, transactionCnt }
    }
    
    findOrCreateHash( hash, blockIdx=0 ){
        const isNew = this.transactionHashes[hash] === undefined
        
        if( isNew )
            // create entry
            this.transactionHashes[hash] = blockIdx>0 ? { index: blockIdx } : { index: -1 }

        const trans = this.transactionHashes[hash]

        if( !isNew && blockIdx > 0 ){
            if( trans.index === -1 ){
                // converting from pending transaction to one in a block
