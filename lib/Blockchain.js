...
export default class Blockchain {
    ...
    static load({ version }) {
        ...
        if( loadBlockIndex < 1 ){
            ...
            const genesisData = Block.buildGenesis() // static call (as instance of block not yet created)
    ...
    static calcMiningReward({ index, update = true }){
    ...
    static addBlock(blockData, options = {}){ 
        ...
        if( prepareOnly ){
            const auditTransactions = TransactionHandler.checkTransactionCount(blockData?.transactions)
            if( auditTransactions.error )
                return error
        ...
        if( !readOnly && (newBlock.minerName === Miner.nodeName || newBlock.minerName === 'genesis' ) )
            TransactionHandler.finalizeTransactions(newBlock)

        const { error, transactionCnt, adminCnt, miningFees }= 
            TransactionHandler.auditTransactions(newBlock.minerName, newBlock.index, newBlock.transactions, this.miningReward)
        ...
        const auditLink = this.auditBlockLinkValid( newBlock )
        if( auditLink.error ){
        ...
        // now sync up the transactions in this block to Mempool, and the Wallet
        const syncResult = TransactionHandler.newBatch( newBlock.transactions, { blockIdx: newBlock.index, txUpdate: true })
        ...
        // successful published to chain: now for any token issue transactions, update Wallet
        if( adminCnt>0 ){
            const issueTransactions = TransactionHandler.filter({ transactions: newBlock.transactions, types: 'tokenIssue' })
            for( const transaction of issueTransactions ){
                const destWallet = Wallet.getUser(transaction.dest, false)
                if( destWallet.error && transaction.type === 'mintIssue' ){
                ...
                debug('cyan',`  > ~ [admin-transaction] found token issue (${transaction.dest}) for ${transaction.amount} tokens. airdropAuth: ${transaction.airdropAuth}`)
                if( !transaction.airdropAuth ) continue
                Wallet.update(transaction.dest, { airdropAuth: transaction.airdropAuth })
            }
        }

        // update height for ledger to know
        Wallet.setMaxBlock( newBlock.index )
    ...
    static auditBlockLinkValid( block ){
    ...
    static auditFullChainValid( chain = this.chain ) {
        ...
        for (let i = 0; i < chain.length; i++) {
            ...
            const result = this.auditBlockLinkValid( block )
            ...
            const auditTransactions = TransactionHandler.auditTransactions(block.transactions,calc.miningReward)
    ...
    static getChain(fromIndex=0,size=0,fields='') {
    ...
    static addChain(chain, blockOptions = {}){
        ...
        if( forceOverwrite && this.height() > fromIndex ){
            ...
            for( const block of dropChain ){
                ...
                const transactions = TransactionHandler.filter({ transactions: block.transactions, type: 'user,admin' })
                Mempool.addQueue( transactions )
            }
            ...
            debug( 'dim' )
            Wallet.resetAllTokens()
            resetWallet = true
            Mempool.reset()

            for( const block of this.chain ){
                let syncResult = TransactionHandler.newBatch( block.transactions, { blockIdx: block.index, txUpdate: true })
                if( syncResult.error ) return syncResult
            }
            debug( 'reset' )
        }
        ...
        Mempool.deleteQueue( blockHashes )
        ...
        return { error: false, hashes: blockHashes, newHashes, addBlockCnt, transactionCnt, adminCnt, miningFees, resetWallet }
    }
}