...
export default class Blockchain {
    constructor({ nodeName, version, ledger, transactionManager, dataPath }) {
        ...
        if( blockData.index < 2 ){
            ...
            const result = this.addBlock({ index: 0, minerName: 'genesis', version: '1.0', transactions: [ genesisTransaction ]})
            if( result.error ){
    ...
    auditBlockValid( currentBlock, blockIsNew = false ){
        ...
        for( const transaction of currentBlock.transactions ){
            ...
            const result = this.ledger.transaction(transaction,{ blockIdx: currentBlock.index, blockIsNew, txUpdate: true })
    ...
    getBlockchain(fromIndex=0,size=0,fields='') {
        ...
        if( fields === 'hashes')
            ...
            result = [ ...chain.map( ({index, minerName, timestamp, fileCache, nodeName, dataPath, powTime}) =>
                        { return {index, minerName, timestamp, fileCache, nodeName, dataPath, powTime}} )]
        else
            // peel off meta-data in block before passing on all data
            result = [ ...chain.map( ({fileCache, nodeName, dataPath, powTime, ...data}) => data )]
        return result
    }
