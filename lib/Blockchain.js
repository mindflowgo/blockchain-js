...
export default class Blockchain {
    ...
    getBlock(index=-1) {
        ...
        // console.log( `[getBlock] height(${this.height()}) -> index(${index})`, JSON.stringify(this.chain.at(index)) )
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( auditResult.error ){
            console.log( `[addBlock] auditBlockValid FAILED on #${auditResult.index}: ${auditResult.error}` )
        ...
        console.log(`  > block #${newBlock.index} ready: imported${transactionCnt>0 ? ` ${transactionCnt} user-transactions` : '' } ${adminCnt>0 ? ` ${adminCnt} admin-transactions,` : '' }${miningFees>0 ? `; $${miningFees} mining fees` : ''} ${ forceOverwrite || !newBlock.fileCache ? ` [wrote file]` : ''} `)
    ...
    auditBlockValid( currentBlock ){
        ...
        if (index > 0) {
            ...
            if (currentBlock.timestamp <= (prevTimestamp - this.timeWindow)) {
                console.log(`Block #${currentBlock.index} timestamp (${currentBlock.timestamp}) is before or equal to previous block (${prevTimestamp} + ${this.timeWindow})` )
                return { error: 'Invalid block timestamp: Must be after previous block' }
            }
            if (currentBlock.timestamp > (time() + this.timeWindow)) {
                console.log(`Block #${currentBlock.index} timestamp (${currentBlock.timestamp}) is too far in the future`)
        ...
        // let debugWallets = []
        // Object.keys(walletKeys).forEach( src => { 
        //     const publicKey = this.ledger.getPublicKey(src)
        //     debugWallets.push( `${this.ledger.wallets[publicKey].name}: $${this.ledger.wallets[publicKey].balance}` )
        // })
        // const walletBalances = this.ledger.walletBalances( Object.keys(walletKeys), true )
        // console.log( ` ~ auditBlockValid #${currentBlock.index} - transactions with wallets (${walletBalances})` )
 
        for( const transaction of currentBlock.transactions ){
            // check transaction itself (signing, balances)
            const transResult = this.ledger.transaction(transaction)
            if( transResult.error ) console.log( `'audit fail Audit on #${currentBlock.index}: ${transResult.error}, wallets: `, this.ledger.wallets )
    ...
    transaction(data, blockIdx = -1) {
        const { isNew, index }= this.findOrCreateHash( data.hash, blockIdx )
        ...
        if( isNew ){
            ...
            if( transaction.error ){
                ...
                if( !data.hash ) this.findOrCreateHash( transaction.hash, blockIdx )
                if( data.hash === transaction.hash )
                    console.log( `     \\ new, with hash, processed ledger transaction`)
                else
                    console.log( `     \\ new, without hash, processed ledger transaction; created hash`)
            }
        } else if( index > -1 ){
            ...
            console.log( `[blockchain::transaction] Should NOT come here: hash(${data.hash} blockIdx(${blockIdx}))`)
        ...
        if( srcWallet ){
            if( data.seq > 0 && blockIdx > -1 && (!transaction.error || transaction.updateSeq) ){
                ...
                console.log( `        + [${data.src.split(':')[0]}]/blockSeq:${data.seq} (!!!) tx.seq(${srcWallet.tx.seq})` )
            }
        }
    ...
    transactionReverse({src, amount, fee, ...data}, blockIdx = -1) {
        ...
        if( blockIdx > -1 && srcWallet.blockSeq.blockIdx.length>0 ){
            ...
            console.log( `[transactionReverse] blockIdx(${blockIdx}), popping last blockIdx(${priorSrcBlockIdx}) from srcWallet:`, srcWallet.blockSeq.blockIdx.pop() )
            ...
            console.log( `   ----> last seq from that block, setting srcWallet.seq = '${seq}':`, srcWallet, block )
    ...
    addBlockchain(chain, blockOptions = {}){
        ...
        if( forceOverwrite && this.height() > fromIndex ){
            ...
            console.log( `    ~ shortening blockchain by ${chainExcessBlocks}`)
        ...
        for( const blockData of chain ){
            ...
            if( addResult.error ) console.log( `   ! error #${blockData.index} > `, addResult )
