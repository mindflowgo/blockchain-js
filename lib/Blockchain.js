...
export default class Blockchain {
    ...
    addBlock(blockData, options = {}){ 
        ...
        if( forceOverwrite || !newBlock.fileCache ){
            // no cache file, so it's a new block, write it!
            newBlock.writeFile(forceOverwrite)
            console.log(` + block #${newBlock.index}: imported (${result.transactionCnt} transactions) & wrote file `+ forceOverwrite ? '[forceOverwrite]' : '')
        } else {
