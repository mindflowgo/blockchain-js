...
export default class Blockchain {
    ...
    removeBlock( blockCnt = 1 ){
        let deleteNum = 0
        do {
            const index = this.chain.length-1 - deleteNum
            const block = this.getBlock(index)
            console.log( `[removeBlock], block:`, block )
            if( block.fileCache ) block.deleteFile()
            // remove that top block
            console.log( `(deleteNum:${deleteNum} < blockCnt:${blockCnt} -- removing top entry: `, this.chain.pop() )
        } while( deleteNum++ < blockCnt )
    }
    ...
    transaction(data, blockIdx = -1) {
        ...
        if( isNew ){
            ...
            transaction = { error: `No transaction: just updated to #${index}${data.seq > 0 ? `; blockSeq(${data.seq})` : '' }`, updateSeq: true, index }
        ...
        if( srcWallet ){
            if( data.seq > 0 && blockIdx > -1 && (!transaction.error || transaction.updateSeq) ){
                ...
                console.log( `        + [${data.src.split(':')[0]}]/blockSeq:${data.seq}` )
            }
        }
    ...
    transactionReverse({src, amount, fee, ...data}, blockIdx = -1) {
        ...
        console.log( `[transactionReverse] blockIdx(${blockIdx}), popping from srcWallet:`, srcWallet.blockSeq.blockIdx.pop() )
        ...
        if( blockIdx > -1 && srcWallet.blockSeq.blockIdx.length>0 ){
            ...
            console.log( `[transactionReverse] updated srcWallet (seq=${seq}):`, srcWallet, block )
            srcWallet.seq = seq
        }
    ...
    addBlockchain(chain) {
        ...
        for( const blockData of chain ){
            const addResult = this.addBlock(blockData)
        ...
        const { transactionCnt, hashes, newHashes }= syncResult
        return { error: false, blocks, hashes, newHashes, addBlockCnt, transactionCnt }
    }
}